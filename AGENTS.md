...
- In this repository, unless the user explicitly specifies another target, "生产环境", "线上", "正式环境", and "部署到禅道" all refer to `ops@192.168.66.42`.
- The default production web container is `zt155-upgrade-web`.
- The application code path inside the container is `/var/www/html`.
- When deploying code changes, prefer syncing only the reviewed files into `zt155-upgrade-web:/var/www/html`; do not overwrite runtime files or directories such as `config/db.php`, `my.php`, `tmp/`, `log/`, or `www/data/`.
- Before any production deployment or online verification, first confirm the target container is running on `192.168.66.42`; if the user names another host, container, or path, follow the user's explicit target instead.
