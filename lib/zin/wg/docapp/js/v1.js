...
const actionsMap =
{
    ...
    doc: function(info)
    {
        ...
        const actions = isInModal ? [] : [
            aiAction,
            aiTeammateAction,
            canViewAITask ? {hint: lang.aiTask, icon: 'task-list', command: `viewAITasks/${doc.id}`} : null,
...
const commands =
{
    ...
    assignAITeammate: function(_, args)
    {
        ...
        const docID      = args[1];
        loadModal($.createLink('aiteammate', 'assignagent', `teammateID=${teammateID}&objectType=doc&objectID=${docID}`), null, {size: 'sm'});
    },
